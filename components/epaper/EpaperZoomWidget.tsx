    ...
    <div className={config.cssClasses || ''} style={parseInlineStyle(config.style)}>
      ...
      <div className={`flex ${orientation === 'vertical' ? 'flex-col' : 'flex-row'} gap-2`}>
        <button
          ...
          disabled={zoom <= 0.5}
          className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
          title="Zoom Out"
        >
          <ZoomOut className="w-5 h-5" />
          {showLabels && <span>Zoom Out</span>}
        </button>

        <button
          onClick={handleReset}
          className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors"
          title="Reset Zoom"
        >
