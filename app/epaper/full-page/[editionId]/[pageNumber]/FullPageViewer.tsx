...
export default function FullPageViewer({ pageData }: { pageData: FullPageData }) {
  ...
  const handleDownload = () => {
    ...
    link.download = `page-${pageData.page_number}.png`;
    link.click();
  };
  ...
  const handleShare = async (platform: string) => {
    ...
    const text = `Check out Page ${pageData.page_number} from ${title}`;
    ...
    if (platform === 'native') {
      if (navigator.share) {
        try {
          ...
          const file = new File([blob], `page-${pageData.page_number}.png`, { type: 'image/png' });
  ...
  const copyLink = async () => {
    try { await navigator.clipboard.writeText(window.location.href); } catch (_e) {}
    setCopied(true);
